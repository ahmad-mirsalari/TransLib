...
DATA_LOCATION FLOAT newClusters[N_CLUSTERS][N_COORDS];
DATA_LOCATION int   newClusterSize[N_CLUSTERS];
//DATA_LOCATION FLOAT* clusters[N_CLUSTERS];
DATA_LOCATION FLOAT  clustersData[N_CLUSTERS*N_COORDS];
DATA_LOCATION int membership[N_OBJECTS];
DATA_LOCATION FLOAT    delta;          /* % of objects change their clusters */
DATA_LOCATION int loop;
#if NUM_CORES > 1
DATA_LOCATION FLOAT local_newClusters[NUM_CORES][N_CLUSTERS][N_COORDS];
DATA_LOCATION int   local_newClusterSize[NUM_CORES][N_CLUSTERS];
DATA_LOCATION FLOAT local_delta[NUM_CORES];
#endif
                    ...
                    FLOAT *coord2)   /* [numdims] */
{
    ...
    if ((N_COORDS & 0x00000001)) {
        FLOAT a = coord1[N_COORDS-1];
        FLOAT b = coord2[N_COORDS-1]; 
        temp[0] += (a-b)*(a-b); 
    }
