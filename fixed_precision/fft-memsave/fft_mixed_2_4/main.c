...
extern short bit_rev_2_4_LUT[FFT_LEN];
...
void main_fn()
{
  ...
  if(get_core_id()==0) {
    ...
    #ifndef SORT_OUTPUT
    for (i=0; i<FFT_LEN; i++)
      bit_rev_2_4_LUT[i] = bit_rev_2_4(i);
    #else
    compute_2_4_LUT();
    #endif
    #endif
  ...
  START_STATS();
  #ifdef PARALLEL
  par_fft_mixed_2_4(Input_Signal, Buffer_Signal_Out);
  #else
  fft_mixed_2_4(Input_Signal, Buffer_Signal_Out);
  #endif
  STOP_STATS();
  EXIT_LOOP_STATS();
