...
int bit_rev_radix4(int);
extern Complex_type twiddle_factors[FFT_LEN_RADIX4/2];
extern short bit_rev_radix4_LUT[FFT_LEN_RADIX4];
...
void main_fn()
{
  ...
  if(get_core_id()==0) {
    ...
    #ifdef BITREV_LUT
      for (i=0; i<FFT_LEN_RADIX4;i++)
        bit_rev_radix4_LUT[i] = bit_rev_radix4(i);
    #endif
  ...
  START_STATS();
  #ifdef PARALLEL
  par_fft_radix4(Input_Signal, Buffer_Signal_Out);
  #else
  fft_radix4(Input_Signal, Buffer_Signal_Out);
  #endif
  STOP_STATS();
  EXIT_LOOP_STATS();
